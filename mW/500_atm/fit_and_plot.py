...
# data: RUN TEMP
temperature_file = 'analysis/freeze_temp_fit_500atm.dat'
...
for run in runs:
	...
	# curve fit
	popt, pcov = curve_fit(sigmoid, data[1], data[2], p0=[0.55, 20, 217, 0.0])
	print (popt)
	print (pcov)
	fit = []
	for x in data[1]:
...
all_runs_plot = 'analysis/fits_500atm.png'

plt.figure(1)
plt.xlim(225,210)
plt.ylim(0,1.0)
plt.title('Heterogeneous freezing on graphite at -500 atm, mW Model, 0.25 K/ns cooling')
...
# plot gradients
all_gradients_plot = 'analysis/gradient_fit_500atm.png'
