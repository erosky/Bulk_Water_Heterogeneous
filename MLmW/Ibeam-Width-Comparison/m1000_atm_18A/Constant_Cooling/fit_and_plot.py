...
# data: RUN TEMP
temperature_file = 'analysis/freeze_temp_fit_m1000atm.dat'
...
for run in runs:
	...
	# curve fit
	popt, pcov = curve_fit(sigmoid, data[1], data[2], p0=[0.55, 20, 250, 0.0])
	print (popt)
	print (pcov)
	fit = []
	for x in data[1]:
...
all_runs_plot = 'analysis/fits_m1000atm.png'

plt.figure(1)
plt.xlim(255,235)
plt.ylim(0,1.0)
plt.title('Heterogeneous freezing on modified graphite potential at -1000 atm, MLmW Model, 0.25 K/ns')
...
# plot gradients
all_gradients_plot = 'analysis/gradient_fit_m1000atm.png'

plt.figure(2)
plt.xlim(255,235)
plt.title('Heterogeneous freezing on modified graphite at -1000 atm, MLmW Model, 0.25 K/ns')
plt.xlabel('Temperature (K)')
