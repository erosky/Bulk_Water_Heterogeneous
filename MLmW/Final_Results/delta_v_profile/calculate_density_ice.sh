...
do
	echo $RUN
	mkdir 1_atm/ice/run_$RUN
	#for timestep in 0
	for timestep in {7950000..8000000..5000}
	do
		output=1_atm/ice/run_$RUN/$timestep.dat
		truncate -s 0 $output
		sed -i -E "22 s/[0-9]+/$timestep/" find_density_ice.awk
		for zbin in {30..55..1}
		do
			sed -i -E "24 s/([0-9]+\.?[0-9]*)|([0-9]*\.[0-9]+)/$zbin/" find_density_ice.awk
			./find_density_ice.awk $DIR/run_$RUN/prod.water.het_freeze_$RUN-240-220.dump >> $output
		done
	done
done
